...
var _blockDetail = _interopRequireDefault(__webpack_require__(/*! ./pages/bbs/blockDetail/blockDetail.vue */ 308));
// @ts-ignore
...
/* harmony import */ var _blockDetail_vue_vue_type_template_id_7b9205a8___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./blockDetail.vue?vue&type=template&id=7b9205a8& */ 309);
/* harmony import */ var _blockDetail_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./blockDetail.vue?vue&type=script&lang=js& */ 311);
...
/* harmony import */ var _blockDetail_vue_vue_type_style_index_0_lang_scss___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./blockDetail.vue?vue&type=style&index=0&lang=scss& */ 313);
...
var render = function () {
  ...
  var g1 = _vm.isTopPost.length
  var g2 = _vm.notTopPost.length
  var g3 = _vm.notTopPost.length
...
var _default = {
  ...
  methods: {
    //获取当前板块的发帖量
    getPostNum: function getPostNum() {
      var _this = this;
      uni.$u.http.get("/bbs/getTargetBlockPostNum", {
        params: {
          block_id: "".concat(this.blockID)
        }
      }).then(function (res) {
        _this.postNum = res.data[0].post_num;
      });
    },
    //获取帖子数据
    getPostData: function getPostData() {
      var _this2 = this;
      uni.$u.http.get("/bbs/getPostByBlock", {
        params: {
          block_id: "".concat(this.blockID)
        }
      }).then(function (res) {
        var data = res.data;
        //筛选出置顶的帖子
        var isTopPost = data.filter(function (item) {
          return item.is_top === 1;
        });
        _this2.isTopPost = isTopPost;
        //筛选出不是置顶的帖子
        var notTopPost = data.filter(function (item) {
          return item.is_top === 0;
        });
        console.log(notTopPost);
        _this2.notTopPost = notTopPost;
      });
    },
    //获取板块详细信息
    getBlockData: function getBlockData() {
      var _this3 = this;
      uni.$u.http.get("/bbs/getBlockInfoByID", {
        params: {
          block_id: "".concat(this.blockID)
        }
      }).then(function (res) {
        _this3.blockData = res.data[0];
        console.log(_this3.blockData);
      });
    },
