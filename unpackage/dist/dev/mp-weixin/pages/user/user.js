...
var _user = _interopRequireDefault(__webpack_require__(/*! ./pages/user/user.vue */ 364));
// @ts-ignore
...
/* harmony import */ var _user_vue_vue_type_template_id_80842834_scoped_true___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./user.vue?vue&type=template&id=80842834&scoped=true& */ 365);
/* harmony import */ var _user_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./user.vue?vue&type=script&lang=js& */ 367);
...
/* harmony import */ var _user_vue_vue_type_style_index_0_id_80842834_lang_scss_scoped_true___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./user.vue?vue&type=style&index=0&id=80842834&lang=scss&scoped=true& */ 369);
...
try {
  components = {
    userTopIcon: function () {
      return Promise.all(/*! import() | components/userTopIcon/userTopIcon */[__webpack_require__.e("common/vendor"), __webpack_require__.e("components/userTopIcon/userTopIcon")]).then(__webpack_require__.bind(null, /*! @/components/userTopIcon/userTopIcon.vue */ 1076))
    },
    uToast: function () {
  ...
  methods: {
    goToDetail: function goToDetail(i) {
      switch (i) {
        case 0:
          uni.navigateTo({
            url: "/pages/user/userFunc/userManage/userManage?user_id=".concat(this.useInfo.user_id)
          });
          break;
        case 1:
          uni.navigateTo({
            url: "/pages/user/userFunc/visitHistory/visitHistory?user_id=".concat(this.useInfo.user_id)
          });
          break;
        case 2:
          uni.navigateTo({
            url: "/pages/user/userFunc/userFollowBlock/userFollowBlock?user_id=".concat(this.useInfo.user_id)
          });
          break;
        case 3:
          uni.navigateTo({
            url: "/pages/user/userFunc/userLikePost/userLikePost?user_id=".concat(this.useInfo.user_id)
          });
          break;
      }
    },
    getNum: function getNum() {
      var _this = this;
      //获取发帖数量
      uni.$u.http.post('/bbs/getPostNumByUserID', {
        user_id: "".concat(this.useInfo.user_id)
      }).then(function (res) {
        _this.userNewsCollect[0].num = res.data[0].post_num;
      }).catch(function (err) {
        console.log(err);
      });
      //获取收藏数量
      uni.$u.http.post('/bbs/getCollectNumByUserID', {
        user_id: "".concat(this.useInfo.user_id)
      }).then(function (res) {
        _this.userNewsCollect[1].num = res.data[0].collect_num;
      }).catch(function (err) {
        console.log(err);
      });
      //获取关注数量
      uni.$u.http.post('/bbs/getFollowNumByUserID', {
        user_id: "".concat(this.useInfo.user_id)
      }).then(function (res) {
        _this.userNewsCollect[2].num = res.data[0].follow_num;
      }).catch(function (err) {
        console.log(err);
      });
      //获取粉丝数量
      uni.$u.http.post('/bbs/getFansNumByUserID', {
        user_id: "".concat(this.useInfo.user_id)
      }).then(function (res) {
        _this.userNewsCollect[3].num = res.data[0].fans_num;
      }).catch(function (err) {
        console.log(err);
      });
    },
    //跳转至关注、收藏等详情页
    toDeatilPage: function toDeatilPage(i) {
      var currentID = uni.getStorageSync("user_id");
      if (i === 0) {
        uni.navigateTo({
          url: "/pages/user/userTags/userPosts/userPosts?user_id=".concat(currentID)
        });
      } else if (i === 1) {
        uni.navigateTo({
          url: "/pages/user/userTags/userCollect/userCollect?user_id=".concat(currentID)
        });
      } else if (i === 2) {
        uni.navigateTo({
          url: "/pages/user/userTags/userFollow/userFollow?user_id=".concat(currentID)
        });
      } else {
        uni.navigateTo({
          url: "/pages/user/userTags/userFans/userFans?user_id=".concat(currentID)
        });
      }
    },
    //去登录
      ...
      if (token) {
        uni.$u.http.get("/users/verifyToken", {
          ...
          if (code == 200) {
            ...
            _this2.isLogin = true;
            _this2.useInfo = res.data.user.tokenData;
            _this2.getNum();
            uni.setStorageSync('user_id', _this2.useInfo.user_id);
          } else {
            uni.showToast({
              title: message,
              icon: 'none'
