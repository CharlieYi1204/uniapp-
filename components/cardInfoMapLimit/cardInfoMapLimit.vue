		...
		<view class="charts-box" >
			<view class="button-box">
				<view class="back-button">
					<u-button  text="返回" @click="registerMap();sendData()" size="mini" v-if="isCity" color="#12B5A1"></u-button>
				</view>
			</view>
				...
				this.$refs.chart.init(echarts,async chart => {
					...
					this.provinceSeriesData.forEach(item=> {
					   ...
					   if(result !== undefined){
						   ...
						   item.itemStyle = {areaColor:"#12B5A1"}
					   }
					   //若不支持，则以#eee用作区分
					   else {
						    item.cardSupport = false
							item.itemStyle = {areaColor:"#eee"}
					   }
					})
									   ...
									   setTimeout( async ()=> {
										  ...
										  // 用于加入支持城市颜色区分判断
											  this.citySeriesData.forEach(item => {
												...
												let result = this.supportDistrict.find(item => item.code === districtCode)
												...
												//筛选出符合条件的区县，以方便传给父组件
												this.supportDistrictData = this.citySeriesData.filter(item => item.cardSupport === true).map( item=> {
													return {
														name:item.name
													}
												})
